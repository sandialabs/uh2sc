...
def UDres(x, U, rho, fluid):
    ...
    """
    fluid.update(CP.PT_INPUTS, x[0], x[1])
    return ((U-fluid.umass())/U)**2 + ((rho-fluid.rhomass())/rho)**2
       
def UDproblem(U, rho, Pguess, Tguess, species, fluid, UDres):
    """
    ...
    if "&" in species:                    
        x0 = [Pguess, Tguess]
        res = minimize(UDres, x0, args=(U, rho, fluid), method='Nelder-Mead', options={'xatol':0.1,'fatol':0.001})
        P1 = res.x[0]
        T1 = res.x[1]
        Ures = U-fluid.umass()
    else:
...
class HydDown:
    ...
    def read_input(self):
        ...
        for wname, well in self.input["wells"].items():            
            self.wells[wname] = Well(well, CP, self.p0, self.T0, self.molefracs, self.comp)
    ...
    def run(self, disable_pbar=True,Tv0=None,Tf0=None):
        ...
        for i in range(1, len(self.time_array)):
            ...
            # setup to run several explicit processes in parallel that only
            # rely on the current state
            func_args[self.axsym.step] = (self.tstep,-self.Q_inner[i])
            func_args[self._energy_balance] = (i,total_mass_rate)
            
            for wname,well in self.wells.items():
                func_args[well.step] = (i,self.P_cavern[i-1])
            
            
            for func, args in func_args.items():
                if self._run_parallel:
                    pool_results[func] = self._pool.apply_async(func,args=args)
                else:
                    pool_results[func] = func(*args)

            if self._run_parallel:
                for func, poolObj in pool_results.items():
                    res_raw = poolObj.get()
                    if func == self.axsym.step:
                        Tgvec = res_raw
                    if func == self._energy_balance:                        
                        self.P_cavern[i] = res_raw[0]
                        self.T_cavern[i] = res_raw[1]
                        self.U_mass[i] = res_raw[2]
                        self.fluid.update(CP.PT_INPUTS, self.P_cavern[i],  self.T_cavern[i])
            else:
                for func, res_raw in pool_results.items():
                    if func == self.axsym.step:
                        Tgvec = res_raw
    ...
    def _energy_balance(self,i,total_mass_rate):
        ...
        U_mass = U_end/self.mass_fluid[i]
            
        P1, T1, U_res = UDproblem(U_mass,self.rho[i],self.P_cavern[i-1],self.T_cavern[i-1], self.species, self.fluid, UDres)
        
        
        
        return P1,T1,U_mass,U_res
