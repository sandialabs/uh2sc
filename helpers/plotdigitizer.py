...
# this is a derivation of the following code under GNU license
# https://github.com/dilawar/PlotDigitizer/blob/main/LICENSE

"""
To select the axes in the application, you need to follow these steps:

    Load an image file by clicking on the "Browse" button and selecting the image file you want to work with.
    The image will be displayed in the main window.
    To select the x-axis, click on the "Change Ax" button to enter axis selection mode.
    Click on the image at the location where you want to set the first point of the x-axis.
    A small text box will appear where you clicked, allowing you to enter the x-coordinate of the point. Enter the value and press Enter.
    Repeat steps 4-5 to set the second point of the x-axis.
    To select the y-axis, click on the "Change Ax" button again to enter axis selection mode.
    Click on the image at the location where you want to set the first point of the y-axis.
    A small text box will appear where you clicked, allowing you to enter the y-coordinate of the point. Enter the value and press Enter.
    Repeat steps 8-9 to set the second point of the y-axis.

Once you have selected both the x and y axes, you can start digitizing data points by clicking on the image. The application will automatically convert the image coordinates to the axis coordinates based on the transformation you defined.
Note that the application uses a logarithmic scale for the axes, so you may need to adjust the scale accordingly when entering the axis coordinates.
...
class MplCanvas(FigureCanvasQTAgg):
    def __init__(self,parent=None,width=5,height=4,dpi=100,zoom_window_size=0.3):
        ...
        self.inset_size = zoom_window_size
...
class MainApplication:
    def __init__(self, image_name,zoom_window_size=0.3):
        self.app = QtWidgets.QApplication(sys.argv)
        self.zoombox_rect = None
        ...
        self.plotwdg = MplCanvas(zoom_window_size=zoom_window_size)
...
#        self.image_data = ImageFile('screenshot_22Jul2020.png')
        self.image_data = ImageFile(image_name)
        ...
        self.win.home_button = QtWidgets.QToolButton(self.win)
        self.win.home_button.setText("Home")
        self.win.home_button.clicked.connect(self.home_cb)
        self.win.toolbar.addWidget(self.win.home_button)

        # Add zoom box button to the toolbar
        self.win.zoombox_button = QtWidgets.QToolButton(self.win)
        self.win.zoombox_button.setText("Zoom Box")
        self.win.zoombox_button.clicked.connect(self.zoombox_cb)
        self.win.toolbar.addWidget(self.win.zoombox_button)

    # Add home and zoom box callbacks
    def home_cb(self):
        self.plotwdg.ax_main.set_xlim(0, self.image_data.width_px)
        self.plotwdg.ax_main.set_ylim(self.image_data.height_px, 0)
        self.plotwdg.fig.canvas.draw()


    def zoombox_button_press_cb(self, event):
        self.zoombox_start_x = event.xdata
        self.zoombox_start_y = event.ydata
        self.zoombox_rect = Rectangle((self.zoombox_start_x, self.zoombox_start_y), 0, 0, fill=False, edgecolor='red', linewidth=2)
        self.plotwdg.ax_main.add_patch(self.zoombox_rect)
        self.plotwdg.fig.canvas.draw()


    def zoombox_motion_cb(self, event):
        if event.xdata is not None and event.ydata is not None and self.zoombox_rect is not None:
            self.zoombox_rect.set_width(event.xdata - self.zoombox_start_x)
            self.zoombox_rect.set_height(self.zoombox_start_y - event.ydata)
            self.plotwdg.fig.canvas.draw()

    def zoombox_button_release_cb(self, event):
        self.state.mode = "select_axes"
        self.plotwdg.ax_main.set_xlim(min(self.zoombox_start_x, event.xdata), max(self.zoombox_start_x, event.xdata))
        self.plotwdg.ax_main.set_ylim(max(self.zoombox_start_y, event.ydata), min(self.zoombox_start_y, event.ydata))
        self.plotwdg.fig.canvas.draw()
        self.plotwdg.ax_main.patches.remove(self.zoombox_rect)
        self.plotwdg.fig.canvas.draw()
        self.plotwdg.fig.canvas.mpl_disconnect(self.plotwdg.fig.canvas.mpl_connect("button_press_event", self.zoombox_button_press_cb))
        self.plotwdg.fig.canvas.mpl_disconnect(self.plotwdg.fig.canvas.mpl_connect("button_release_event", self.zoombox_button_release_cb))
        self.plotwdg.fig.canvas.mpl_disconnect(self.plotwdg.fig.canvas.mpl_connect("motion_notify_event", self.zoombox_motion_cb))


    def zoombox_cb(self):
        self.state.mode = "zoombox"
        self.plotwdg.fig.canvas.mpl_connect("button_press_event", self.zoombox_button_press_cb)
        self.plotwdg.fig.canvas.mpl_connect("button_release_event", self.zoombox_button_release_cb)
        self.plotwdg.fig.canvas.mpl_connect("motion_notify_event", self.zoombox_motion_cb)
    ...
    def textbox_finished_cb(self):
        ...
        if self.textbox.text() == "":
            ...
            if len(pvec) != 2:
                self.label_tb.setText("Invalid input. Please enter x,y coordinates.")
                return
            try:
                p1 = Coordinate(float(pvec[0].strip()), float(pvec[1].strip()))
            except ValueError:
                self.label_tb.setText("Invalid input. Please enter numbers only.")
                return
