...
def create_df_from_sim_output(nonleapyear,results_dict):
    ...
    epoch = datetime(nonleapyear,1,1)
    result = [pd.Timestamp(epoch + timedelta(seconds=second_of_year)) for second_of_year in df["Time (s)"]]
...
def fahrenheit_to_kelvin(degF):
    return 5/9 * (degF - 32) + 273.15
...
class Nielson2008_Cavern_Case(object):
    ...
    def __init__(self):
        ...
        overburden_pressure = con.gravitational_constant * (1000 * con.feet_to_meters 
                                                        * 144 * con.lbs_ft3_2_kg_m3
                                                    + 2000 * con.feet_to_meters
                                                        * 135 * con.lbs_ft3_2_kg_m3) #Pa  ~ pressure at 1000 m 
        ...
        temp_increase_with_depth = 0.012 / con.scale_factor_between_kelvin_and_rankin / con.feet_to_meters
        avg_ground_temperature = surface_temperature + temp_increase_with_depth * (self.cavern_top_depth + self.cavern_height / 2)
...
def cycle_flow_commands(sc):
    ...
    new_inp["wells"]["cavern_well"]["valves"]["inflow_mdot"]["time"] = [tstep*idx for idx in range(new_nstep+1)]
    new_inp["wells"]["cavern_well"]["valves"]["inflow_mdot"]["mdot"] = [new_mdot for idx in range(new_nstep+1)]
...
class Test_SaltCavern_Verification(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        # load constants for the Nielson paper.
        cls.nielson_obj = Nielson2008_Cavern_Case()
    ...
    def test_verification_(self):
        ...
        """
        This test takes ~200-300 seconds and is a validation test that compares to Nielson's model results 
        for a salt cavern.
            ...
            for gas_type,subd in study_input.items():
                 ...
                 for days_per_cycle in days_per_cycle_list:
                    ...
                    if np.floor(nstep) != nstep:
                        raise ValueError("You must make the cavern_time_step an integer multiple of the tstep!")
                    else:
                        nstep = int(nstep)
                    ...
                    T_max_pressure = fahrenheit_to_kelvin(verify_obj["degrees fahrenheit"].max())
                    T_min_pressure = fahrenheit_to_kelvin(verify_obj["degrees fahrenheit"].min())
                    
                    rho_max_pressure = PropsSI('D','T',T_max_pressure,'P',self.nielson_obj.max_avg_pressure,gas_type)
                    rho_min_pressure = PropsSI('D','T',T_min_pressure,'P',self.nielson_obj.min_avg_pressure,gas_type)
                    ...
                    inp["initial"]["pressure"] = self.nielson_obj.max_avg_pressure # 30e6 Pa is approximately 1000 m deep overburden pressure
                    inp["initial"]["fluid"] = gas_type
                    inp['initial']['temperature'] = subd['initial_temperatures'][days_per_cycle]
    
                    # this just takes single steps
                    inp["wells"]["cavern_well"]["valves"]["inflow_mdot"]["time"] = [con.tstep*idx for idx in range(nstep+1)]
                    inp["wells"]["cavern_well"]["valves"]["inflow_mdot"]["mdot"] = [mdot for idx in range(nstep+1)]
                    ...
                    if self.print_figures:
                        ...
                        s_kelvin.plot(ax=ax,label="Gas Nielson",linestyle="None",marker="x",markersize=10)
                    ...
                    ybar_nielson = np.interp(time_s_nielson,v_df['Time (s)'].values,v_df['Gas temperature (K)'].values)
                    ...
                    msg = "The verification match between Nielson 2008 and this model has degraded in comparison to the 12/8/2023 original check."
        
                    #
                    if self.print_msg:
                        print("++++++++++++++++++++++++++++++++++")
                        print(gas_type + ":" + str(days_per_cycle) + ":\n\n")
                        print("max:" + str(max_error) + " < " + str(subd['error'][days_per_cycle]['max']))
                        print("min:" + str(min_error) + " < " + str(subd['error'][days_per_cycle]['min']))
                        print("mean:" + str(mean_error) + " < " + str(subd['error'][days_per_cycle]['mean']))
                        print("------------------------------------------")
...
if __name__ == "__main__":
    ...
    o = unittest.main(Test_SaltCavern_Verification())
